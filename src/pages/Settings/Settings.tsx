...
export const Settings = () => {
  ...
  const [passwordStrength, setPasswordStrength] = useState({ label: "", color: "#ccc", width: "0%" });
  ...
  const onChangeHandler = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>, name: keyof FormControls) => {
    ...
    Object.keys(updatedControls).forEach(key =>
      (formIsValid = updatedControls[key as keyof FormControls].valid && formIsValid)
    );
  ...
  const submitHandler = async (e: React.FormEvent) => {
    ...
    if (formControls.password.value) {
      ...
      if (!minLength || !hasUppercase || !hasLowercase) return setError("Password must be at least 8 characters with upper and lowercase letters");
      if (formControls.password.value !== formControls.passwordConfirmation.value) return setError("Passwords do not match");
    ...
    try {
      ...
      if (res.data.message === "Confirm your new email") {
        setInfoMessage("Confirmation sent to new address");
      } else {
  ...
  useEffect(() => { if (!user || user.role === "blocked") navigate("/"); }, [user, navigate]);
